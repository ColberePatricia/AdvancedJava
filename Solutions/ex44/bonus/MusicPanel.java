...
public class MusicPanel extends JPanel {
	 ...
	 *  2.  Show this dialog
	 *  3.  When the dialog returns, check if the "Ok" button was pressed.
	...
	class EntryActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent event) {
			...
			MusicEntryDialog myEntryDialog = new MusicEntryDialog(parentFrame, categoryArrayList);
			...
			if (myEntryDialog.isOkButtonPressed()) {
				...
				// get the recording
				MusicRecording theRecording = myEntryDialog.getMusicRecording();
				
				// add it to data accessor
				myDataAccessor.addRecording(theRecording);

				//
				//  BONUS WORK
				//
				JOptionPane.showMessageDialog(MusicPanel.this, "Recording added successfully!");
				
				// populate the list box by selecting an item in the combo box.
				//
				// the combo box has a listener that will populate the list box
				// when categories are selected.
				String theCategory = theRecording.getCategory();
				categoryComboBox.setSelectedItem(theCategory);
			}			
		}	
	}
}